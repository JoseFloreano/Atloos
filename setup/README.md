...
|---|---|---|---|---|---|
| `graphiti/.env` | `%LOCALAPPDATA%\graphiti\.env`<br>*(Unix: `~/.local/share/graphiti/.env`)* | `/etc/atloos/graphiti.env` | `DEEPSEEK_API_KEY`, `OPENAI_API_KEY` (embedder, obligatoria), `GOOGLE_API_KEY`/`GROQ_API_KEY`/`ANTHROPIC_API_KEY` (rutas alternativas), `FALKORDB_PASSWORD` | `docker compose --env-file` → MCP server + FalkorDB | DeepSeek: platform.deepseek.com → API keys → crear nueva y borrar la vieja. OpenAI: platform.openai.com → API keys → revoke. Gemini: aistudio.google.com. Groq: console.groq.com. `FALKORDB_PASSWORD`: la eliges tú; cámbiala y reinicia el contenedor |
| `claude-telegram/.env` | `%LOCALAPPDATA%\claude-telegram\.env`<br>*(Unix: `~/.config/claude-telegram/.env`)* | `/etc/atloos/telegram.env` | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_ALLOWED_USER_ID` | `notify_telegram.py` (T0) y `tg_daemon.py` (T1) | Token: **@BotFather → `/revoke`** invalida el viejo al instante y da uno nuevo. `TELEGRAM_CHAT_ID` y `TELEGRAM_ALLOWED_USER_ID` **no son secretos rotables** — son identificadores tuyos; si cambian, se editan a mano |
