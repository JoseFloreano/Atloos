...
| 7 | **W2:** el gate rechaza un merge sin verde en prueba deliberada | ⬜ **abierto** | **Prueba hecha el 2026-08-07: 2/4, no cierra** ([[2026-08-07-prueba-gate]]). E2 (verde apoyado en tests tocados) y E3 (hash falso) rechazaron con `main` intacta; E1 y E4 mergearon a `main` **sin confirmación humana**. Causa medida: en 3 de 4 escenarios **la skill no llegó a cargarse** — ganó `superpowers:finishing-a-development-branch`, que no tiene paso 6 ni squash. La sonda A/B lo aísla: la misma situación de E4 con la palabra "frente" en el prompt sí dispara el gate, y entonces ejecuta los 7 pasos correctamente. **El gate no falla: no corre.** Sigue abierto hasta arreglar el trigger y repetir (E1 además debe rediseñarse: su commit posterior al verde no rompía la suite, así que no discriminaba) |
